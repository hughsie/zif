...
G_DEFINE_TYPE (ZifSackLocal, zif_sack_local, ZIF_TYPE_SACK)

/**
 * zif_sack_local_finalize:
 **/
static void
zif_sack_local_finalize (GObject *object)
{
	ZifSackLocal *sack;

	g_return_if_fail (object != NULL);
	g_return_if_fail (ZIF_IS_SACK_LOCAL (object));
	sack = ZIF_SACK_LOCAL (object);

	G_OBJECT_CLASS (zif_sack_local_parent_class)->finalize (object);
}

/**
 * zif_sack_local_class_init:
 **/
static void
zif_sack_local_class_init (ZifSackLocalClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	object_class->finalize = zif_sack_local_finalize;
}
 ...
 **/
static void
zif_sack_local_init (ZifSackLocal *sack)
{
	ZifStoreLocal *store;
	store = zif_store_local_new ();
	zif_sack_add_store (ZIF_SACK (sack), ZIF_STORE (store));
 ...
 **/
ZifSackLocal *
zif_sack_local_new (void)
{
	ZifSackLocal *sack;
	sack = g_object_new (ZIF_TYPE_SACK_LOCAL, NULL);
	return ZIF_SACK_LOCAL (sack);
}
...
void
zif_sack_local_test (EggTest *test)
{
	ZifGroups *groups;
	ZifSackLocal *sack;
	ZifStoreLocal *store;
	...
	if (!egg_test_start (test, "ZifSackLocal"))
		return;
