 ...
 *
 * Return value: gchar pointer array of category names
 **/
GPtrArray *
				  ...
				  ZifCompletion *completion, GError **error)
{
	...
	for (i=0; i<len; i++) {
		...
		g_ptr_array_add (array, g_strdup (data->id));
	}
out:
	return array;
}
 ...
 *
 * Return value: gchar pointer array of group names
 **/
GPtrArray *
...
zif_repo_md_comps_test (EggTest *test)
{
	...
	ret = zif_repo_md_set_mdtype (ZIF_REPO_MD (md), ZIF_REPO_MD_TYPE_COMPS);
	if (ret)
		egg_test_success (test, NULL);
	else
		egg_test_failed (test, "failed to set");
	...
	/************************************************************/
	egg_test_title (test, "get groups for apps");
